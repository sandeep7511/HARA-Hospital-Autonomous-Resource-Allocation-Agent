        ...
        if not t_name or not t_cond:
            ...
            if not reg.get("success"):
                ...
                if not triage.get("success"):
                    ...
                    st.markdown(f"""
<div style="background:{color};border-radius:10px;padding:16px 20px;margin:12px 0;">
  <h3 style="margin:0">{icon} {triage['severity_label']} — Severity {sev}/5</h3>
...
""", unsafe_allow_html=True)
                    ...
                    if include_vitals:
                        ...
                        vc = st.columns(7)
                        vc[0].metric("Temp",       f"{v_temp}°C")
                        vc[1].metric("BP",         f"{v_bps}/{v_bpd}")
                        vc[2].metric("HR",         f"{v_hr} bpm")
                        vc[3].metric("SpO₂",       f"{v_spo2}%")
                        vc[4].metric("Height",     f"{v_height} cm")
                        vc[5].metric("Weight",     f"{v_weight} kg")
                        vc[6].metric("BMI",        bmi_est)
                    ...
                    with col_b:
                        ...
                        if imaging:
                            ...
                            st.info(
                                f"**{imaging}** required. "
                                "Check Ops dashboard to confirm machine availability "
                                "and book scan or trigger inter-hospital referral."
                            )
                    ...
                    # Save result to session state — no rerun, report stays visible
    ...
    # ── Persistent triage report display ─────────────────────────────
    if st.session_state.triage_result:
        ...
        # Header card
        st.markdown(f"""
<div style="background:{color};border-radius:10px;padding:16px 20px;margin:8px 0;">
  <h3 style="margin:0">{icon} {triage['severity_label']} — Severity {sev}/5</h3>
  <p style="margin:6px 0 0;color:#374151">
    <b>Referred to:</b> {triage['doctor_referral']}
    {"&nbsp;·&nbsp;<b>Specialist:</b> " + specialist if specialist else ""}
    ...
    if all_p_lookup:
        ...
        if st.button("Load Report"):
            ...
            if report and report.get("success"):
                ...
                icon  = SEV_ICON.get(sev, "⚪")
                st.markdown(f"""
<div style="background:{color};border-radius:10px;padding:14px 18px;margin:10px 0;">
  <h4 style="margin:0">{icon} {report['severity_label']} — Severity {sev}/5</h4>
  ...
  <p style="color:#6b7280;font-size:13px"><em>{report['gemini_reasoning']}</em></p>
</div>
""", unsafe_allow_html=True)
                # Vitals
